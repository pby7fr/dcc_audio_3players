...
// Version 5.5  Geoff Bunza 2014,2015,2016,2017
 ...
 *  Pro Mini Transmit (TX) connected to DFPlayer Receive (RX)  Pin 2 via 470 Ohm Resistor
 ...
 *  F6 plays a random selection in random order of tracks 1-6. 
...
const int FunctionPin10 = 20;   // Place holders
    ...
    switch (Dcc.getCV( 30+(i*5))) {
      ...
      case 3:
	   ...
	   case 5:   // Fade On
         ...
         break;         
       case 6:   // Audio Track Play
         if (digitalRead(busy_pin)== LOW) ftn_queue[i].inuse = 0;
         audio_on = 0;
         break;
...
#define fadedelay 24
    ...
    case 6:    // Audio Play
      if (audio_on == 0) {    // Audio Off = Not Playing
        ftn_queue[function].inuse = FuncState;
        if (FuncState==1) {   // Audio was off but function found is on
          audio_on = 1;       // so start playing this clip
          mp3_set_volume (ftn_queue[function].increment);
          delay(5);
          mp3_play (ftn_queue[function].start_value);  //  play clip function+1
        }
      }
      break;
    case 7:    // Future Function
      if (audio_on == 0) {    // Audio Off = Not Playing
        ftn_queue[function].inuse = FuncState;
        if (FuncState==1) {   // Audio was off but function found is on
          audio_on = 1;       // so start playing this clip
          mp3_set_volume (ftn_queue[function].increment);
          delay(5);
          mp3_play (random(0,num_clips+1));  //  play random clip
        }
      }
      break;  
    default:
